...
public class Throwing : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(!Paused() && playerState.states != PlayerStates.Dead){
            if(Input.GetKey(throwKey) && readyToThrow && totalThrows > 0)
            {
                ...
                if(!playerState.playerIsSafe){
                    projectile.GetComponent<FlipFlops>().canGivePoints = false;
                }else { projectile.GetComponent<FlipFlops>().canGivePoints = true; }
